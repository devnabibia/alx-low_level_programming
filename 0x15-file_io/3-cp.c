...
/**
 * main - program that copies the content of a file to another fi
 * @argv: string argment
 * @argc: num arg
 * Return: 0
 */
...
int main(int argc, char *argv[])
{
	...
	if (argc != 3)
		pritf(STDERR_FILENO, "Usage: cp file_from file_to\n"), exit(97);
	file_from = open(argv[1], 0_RDONLY);
	if (file_from == -1)
	{
		print(STDERR_FILENO, "Error: Can't write to %s\n", argv[1]);
		exit(98);
	}
	file_to = open(argv[2], 0_WRONGLY | 0_CREAT | 0_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH);
	if (file_to == -1)
	{
		print(STDERR_FILENO, "Error: Can't write to %s\n", argv[2]);
